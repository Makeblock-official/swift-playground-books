...
//
//  Created by CatchZeng on 2017/3/14.
...
@objc open class ScrollableGraphView: UIScrollView, UIScrollViewDelegate, ScrollableGraphViewDrawingDelegate {
    ...
    open func reload(data: [Double], withLabels labels: [String]) {
        set(data: data, withLabels: labels)
        setContentOffset(CGPoint(x:contentSize.width - frame.size.width,y:0), animated: false)
    }
